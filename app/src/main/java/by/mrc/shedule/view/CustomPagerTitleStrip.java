...
public class CustomPagerTitleStrip extends ViewGroup {
    ...
    public void setup(ViewPager2 pager, TitleProvider titleProvider) {
        pager.registerOnPageChangeCallback(mPageListener);
        mPager = pager;
        this.titleProvider = titleProvider;
        updateText(mPager.getCurrentItem());
    }
    ...
    protected void onDetachedFromWindow() {
        ...
        if (mPager != null) {
            mPager.unregisterOnPageChangeCallback(mPageListener);
            mPager = null;
        }
    }

    void updateText(int currentItem) {
        final int itemCount = titleProvider.getSize();
        mUpdatingText = true;

        CharSequence text = null;
        if (currentItem >= 1) {
            text = titleProvider.getName(currentItem - 1);
        }
        mPrevText.setText(text);

        mCurrText.setText(currentItem < itemCount
                ? titleProvider.getName(currentItem) : null);
